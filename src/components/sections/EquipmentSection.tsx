...
export default function EquipmentSection({ equipment }: EquipmentSectionProps) {
  const categorizedEquipment = {
    ...
    professional: equipment.filter(item => item.category === 'professional')
  };

  const renderEquipmentList = (items: Equipment[], title: string) => (
    <div className="space-y-4">
      <h3 className="font-semibold text-[#7E69AB] text-lg">{title}</h3>
  ...
  return (
    <Card className="border-2 border-[#D6BCFA] bg-white/80 backdrop-blur-sm">
      ...
      <CardContent className="space-y-6">
        ...
        {categorizedEquipment.professional.length > 0 && 
          renderEquipmentList(categorizedEquipment.professional, "Professional Equipment")}
